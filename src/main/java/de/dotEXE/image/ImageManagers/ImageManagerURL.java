...
public class ImageManagerURL implements Listener {

    private static ImageManagerURL instance = null;

    public static ImageManagerURL getInstance() {
        if (instance == null)
            instance = new ImageManagerURL();
        return instance;
    }

    private final CConfig dataFile = new CConfig("URLMaps", Main.getPlugin());
    ...
    public void onMapInitEvent(MapInitializeEvent event) {
        if (hasImage(event.getMap().getId())) {
            ...
            view.addRenderer(new ImageRendererURL(getArgs(view.getId())));
